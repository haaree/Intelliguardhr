...
const LeaveManagementEnhanced: React.FC<LeaveManagementEnhancedProps> = ({
  data,
  onUpdate,
  onPushToMonthly,
  role,
  ...
  const handleLeaveUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Extract unique statuses from the uploaded file
        const statuses = Array.from(new Set(leaves.map(l => l.status).filter(s => s)));
  ...
  const handleReconcile = () => {
    if (selectedStatuses.length === 0) {
      alert('Please select at least one status to reconcile.');
      return;
    }
    ...
    try {
      // Filter leave records by selected statuses
      const filteredLeaves = localLeaves.filter(leave =>
        selectedStatuses.includes(leave.status)
      );
      ...
      absentRecords.forEach(absent => {
        ...
        reconciliations.push({
          ...
          remarks: matchingLeave
            ? `${matchingLeave.leaveType} - ${matchingLeave.reason} (Status: ${matchingLeave.status})`
      ...
      addAuditLog(
        ...
        undefined,
        `${reconciliations.filter(r => r.reconciliationStatus === 'Matched').length} matched`,
        `Reconciled for statuses: ${selectedStatuses.join(', ')}`
      );

      alert(`Reconciliation complete!\n\nFound ${reconciliations.filter(r => r.reconciliationStatus === 'Matched').length} matches out of ${reconciliations.length} absent records.\n\nOnly records with selected statuses (${selectedStatuses.join(', ')}) were reconciled.`);
    } catch (err) {
      console.error(err);
  ...
  const handleApprove = (reconciliation: LeaveReconciliation) => {
    ...
    addAuditLog(
      'Approve',
      reconciliation.employeeNumber,
      reconciliation.employeeName,
      'Matched',
      'Approved',
      `Date: ${reconciliation.date}, Final Status: ${reconciliation.finalStatus}`
    );
  };
  ...
  const handleStatusUpdate = (reconciliation: LeaveReconciliation, newStatus: string) => {
    const updated = localReconciliations.map(r =>
      r.employeeNumber === reconciliation.employeeNumber && r.date === reconciliation.date
        ? { ...r, finalStatus: newStatus, remarks: `Status updated to ${newStatus} by ${currentUser}` }
        : r
    );
    setLocalReconciliations(updated);
  ...
  const handleSaveAndPush = () => {
    if (confirm('Save reconciliations and update Monthly Report with approved leaves?\n\nOnly APPROVED reconciliations will be pushed to the monthly report.\nOther statuses will remain blank in the monthly report.')) {
      // Mark approved reconciliations as pushed
      const updatedReconciliations = localReconciliations.map(r =>
        r.reconciliationStatus === 'Approved'
          ? { ...r, isPushedToMonthly: true }
          : { ...r, isPushedToMonthly: false }
      );

      setLocalReconciliations(updatedReconciliations);
      onUpdate(localLeaves, updatedReconciliations, localLogs);

      // Add audit log
      const approvedCount = updatedReconciliations.filter(r => r.isPushedToMonthly).length;
      addAuditLog(
        'Push to Monthly',
        'BULK',
        `${approvedCount} approved reconciliations`,
        undefined,
        `Pushed to monthly report`,
        `Total reconciliations: ${updatedReconciliations.length}, Pushed: ${approvedCount}`
      );

      onPushToMonthly();
      alert(`Leave reconciliations saved!\n\n✓ ${approvedCount} approved reconciliations pushed to Monthly Report\n✗ ${updatedReconciliations.length - approvedCount} other records will show as blank in Monthly Report`);
    }
  };
  ...
  const handleExport = () => {
    if (viewMode === 'leaves') {
      ...
      const ws = XLSX.utils.json_to_sheet(filteredReconciliations.map(r => ({
        ...
        'Leave Status': r.leaveRecord?.status || 'N/A',
  ...
  const stats = useMemo(() => {
    return {
      ...
      pushed: localReconciliations.filter(r => r.isPushedToMonthly).length
    };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium text-xs uppercase tracking-widest mt-1">
            {localLeaves.length} Leave Records • {localReconciliations.length} Reconciliations • {stats.pushed} Pushed to Monthly
          </p>
        </div>
          ...
          {isAdmin && (
            <>
              <button
                ...
                disabled={localLeaves.length === 0 || selectedStatuses.length === 0 || isProcessing}
      ...
      {showStatusSelection && availableStatuses.length > 0 && (
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-6 rounded-3xl border border-indigo-100 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Filter className="text-indigo-600" size={20} />
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">
                Select Leave Statuses for Reconciliation
              </h3>
            </div>
            <div className="flex gap-2">
              <button
                onClick={selectAllStatuses}
                className="text-xs font-black text-indigo-600 hover:text-indigo-700 px-3 py-1.5 bg-white rounded-lg border border-indigo-200 hover:border-indigo-300 transition-all"
              >
                Select All
              </button>
              <button
                onClick={clearAllStatuses}
                className="text-xs font-black text-slate-600 hover:text-slate-700 px-3 py-1.5 bg-white rounded-lg border border-slate-200 hover:border-slate-300 transition-all"
              >
                Clear All
              </button>
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            {availableStatuses.map(status => (
              ...
              >
                {selectedStatuses.includes(status) && <CheckCircle size={14} className="inline mr-2" />}
                {status}
              </button>
            ))}
          </div>
          <p className="text-xs text-slate-600 mt-4 font-medium">
            {selectedStatuses.length > 0
              ? `Selected ${selectedStatuses.length} status(es): ${selectedStatuses.join(', ')}`
              : 'No statuses selected. Please select at least one status to proceed with reconciliation.'}
          </p>
        </div>
      )}
            ...
            <table className="w-full text-left table-auto border-collapse">
              <thead className="bg-slate-900 text-white text-[9px] uppercase font-black tracking-widest sticky top-0 z-40">
                <tr>
                  ...
                  <th className="px-6 py-5">Leave Status</th>
                  <th className="px-6 py-5">Reconciliation</th>
                  ...
                  <tr>
                    <td colSpan={isAdmin ? 10 : 9} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-xs text-slate-400">Select statuses and click "Reconcile" to match leave records</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  filteredReconciliations.map((rec, idx) => (
                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {rec.leaveRecord ? (
                          <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${
                            rec.leaveRecord.status === 'Approved' ? 'bg-emerald-100 text-emerald-700' :
                            rec.leaveRecord.status === 'Rejected' ? 'bg-rose-100 text-rose-700' :
                            'bg-amber-100 text-amber-700'
                          }`}>
                            {rec.leaveRecord.status}
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-4">
                        {isAdmin && rec.reconciliationStatus === 'Approved' && !rec.isPushedToMonthly ? (
                          <select
                            value={rec.finalStatus}
